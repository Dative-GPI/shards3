...
import { computed, defineComponent, onMounted, ref, PropType, toRefs, watch } from "vue";

import { FSDataTableColumn, FSDataTableFilter } from "@dative-gpi/foundation-shared-components/models";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const headers = computed((): FSDataTableColumn[] => {
      ...
      return sortedVisibleColumns.map((c): FSDataTableColumn => {
    ...
    const toggleFilters = (header: string, value: FSDataTableFilter[]) => {
    ...
    const computeFilters = () => {
      ...
      const filterDict: { [key: string]: FSDataTableFilter[] } = {};
    ...
    const filterItem = (values: FSDataTableFilter[], property: any, item: any): boolean => {
